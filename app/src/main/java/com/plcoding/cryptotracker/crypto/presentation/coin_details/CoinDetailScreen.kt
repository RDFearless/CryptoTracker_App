...
import com.plcoding.cryptotracker.crypto.presentation.models.toDisplayableFormat
...
fun CoinDetailScreen(
    ...
    if(coinState.isLoading) {
    ...
    } else if(coinState.selectedCoin != null) {
        val coin = coinState.selectedCoin
        ...
        Column(
            ...
            val color = if(coin.changePercentage24Hr.value < 0.0) {
                Color.Red
            } else {
                if(isSystemInDarkTheme()) Color.Green else greenBackground
            }

            val icon = if(coin.changePercentage24Hr.value < 0.0) {
            ...
            FlowRow(
                ...
                InfoCard(
                    ...
                    defaultTextStyle = LocalTextStyle.current.copy(
                        fontSize = 13.sp
                    )
                )
                val text = (coin.priceUsd.value * (coin.changePercentage24Hr.value / 100)).toDisplayableFormat()

                InfoCard(
                    title = stringResource(id = R.string.change_percent_last24hr),
            ...
            // LINE CHART
            AnimatedVisibility(
                visible = coin.coinPrice.isNotEmpty()
            ) {
                ...
                val startIndex = (coin.coinPrice.lastIndex - amountOfVisibleDataPoints)
                    .coerceAtLeast(0)
                LineChart(
                    ...
                    visibleDataPointIndices = startIndex..coin.coinPrice.lastIndex,
                    unit = "$",
                    modifier = Modifier
                        .fillMaxWidth()
                        .aspectRatio(16 / 9f)
